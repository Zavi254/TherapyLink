...
export const availabilitySchema = z.object({
    hourlyRate: z.coerce.number().min(50, 'Hourly rate must be atleast $50').max(1000, 'Hourly rate cannot exceed $10000'),
    schedule: z.record(
        z.object({
            enabled: z.boolean(),
            timeBlocks: z.array(
        ...
        (schedule) => {
            // Check if at least one day is enabled with time blocks
            return Object.values(schedule).some(
                (day) => day.enabled && day.timeBlocks.length > 0
            );
        },
        {
            error: 'Please set availability for atleast one day',
        }
    ),
});